...
apps/       one app on top of the VNC-only XFCE parent — what you'll usually add
...
layer: apps
description: XFCE desktop with MyApp, auto-started.
version: "1.0.0"       # bump on ANY change — tags are immutable
from: core-ubuntu-noble-xfce   # the parent image
...
When the app is "a list of apt packages plus an autostart entry", skip
the Dockerfile entirely:

```yaml title="apps/myapp/manifest.yaml"
name: myapp
layer: apps
description: XFCE desktop with MyApp.
version: "1.0.0"
from: core-ubuntu-noble-xfce
recipe:
  apt: [myapp, myapp-plugins]
  autostart: myapp          # XFCE autostart entry
  # startup: <cmd>          # alternative: single-app on the bare base (WAAS_STARTUP)
...
3. Autostart: ship an `/etc/xdg/autostart/waas-app.desktop` for a full
   desktop, or set `ENV WAAS_STARTUP="<command>"` for a single-app
   image on the bare base.
